...
{
    std::cout << strsignal(signum) << " Signal is received! Terminate RealSense Component...\n";
...
RealSenseCameraComponent::RealSenseCameraComponent(const rclcpp::NodeOptions& options)
    : Node("realsense_camera_component",
      ...
      _intialize_time_base(false)
{
    ...
    signal(SIGINT, signalHandler); // Ctrl+Cで停止させた時などの処理
...
void RealSenseCameraComponent::onInit()
{
    getParameters();
    setupDevice();
    setupPublishers();
    ...
    timer_ = this->create_wall_timer(std::chrono::seconds(1), std::bind(&RealSenseCameraComponent::publishStaticTransforms, this));
    RCLCPP_INFO(logger_, "RealSense Component Is Up!");
}

void RealSenseCameraComponent::getParameters()
{
...
void RealSenseCameraComponent::setupDevice()
{
    ...
    try
    {
        ...
        RCLCPP_INFO(logger_, "My check : %s", "1"); //!!!!!

        auto list = _ctx->query_devices();
        RCLCPP_INFO(logger_, "My check : %s", "2"); //!!!!!
        if (0 == list.size())
        {
            _ctx.reset();
            RCLCPP_ERROR(logger_, "No RealSense devices were found! Terminate RealSense Component...");
            rclcpp::shutdown();
            exit(1);
        }
        else
        {                                                       //!!!!!
            RCLCPP_INFO(logger_, "list size: %i", list.size()); //!!!!!
        }
        RCLCPP_INFO(logger_, "My check : %s", "3"); //!!!!!

        // Take the first device in the list.
        // Add an ability to get the specific device to work with from outside.
        _dev = list.front();
        RCLCPP_INFO(logger_, "My check : %s", "4"); //!!!!!
        _ctx->set_devices_changed_callback([this](rs2::event_information &info) {
            if (info.was_removed(_dev))
            {
                RCLCPP_FATAL(logger_, "The device has been disconnected! Terminate RealSense Component...");
                rclcpp::shutdown();
                exit(1);
            }
        });
        ...
        for (auto &&elem : dev_sensors)
        {
            ...
            else
            {
                RCLCPP_ERROR(logger_,
                             "Module Name \"%s\" isn't supported by LibRealSense! Terminate RealSense Component...",
                             module_name);
                rclcpp::shutdown();
                exit(1);
            }
            RCLCPP_INFO(logger_, "%s was found.", std::string(elem.get_info(RS2_CAMERA_INFO_NAME)).c_str());
        }
...
void RealSenseCameraComponent::setupPublishers()
{
...
void RealSenseCameraComponent::setupStreams()
{
    ...
    try
    {
        ...
        auto frame_callback = [this](rs2::frame frame) {
            ...
            {
                // std::cout << "get frame!" << std::endl; // comment 2020.3.16
                // std::cout << " RS2_STREAM_COLOR: " << RS2_STREAM_COLOR << std::endl; // comment 2020.3.16
                // std::cout << " RS2_STREAM_DEPTH: " << RS2_STREAM_DEPTH << std::endl; // comment 2020.3.16
                ...
                for (auto it = frameset.begin(); it != frameset.end(); ++it)
                {
                    ...
                    // RBG画像とDepth画像のパブリッシュ
                    publishFrame(f, t);
                }

                // alignされたdepth image のパブリッシュ //comment 2020.3.16
                if (_align_depth && is_depth_frame_arrived && is_color_frame_arrived)
                {
                    //std::cout << "publishAlignedDepthTopic" << std::endl; //comment 2020.3.16
                    RCLCPP_DEBUG(logger_, "publishAlignedDepthTopic(...)");
                    publishAlignedDepthImg(frame, t);
                }

                // point cloud topic のパブリッシュ //comment 2020.3.16
                ...
                // alignされたpoint cloud topic のパブリッシュ //comment 2020.3.16
        ...
        for (auto &streams : IMAGE_STREAMS)
        {
            ...
            if (!profiles.empty())
            {
                ...
                if (DEPTH == stream)
                {
                    ...
                    std::cout << "depth = " << _depth_scale_meters << std::endl;
                }
            ...
            accel_profile != _enabled_profiles.end())
        {
            ...
            sens->start([this](rs2::frame frame) {
                ...
                //    0 != _imu_publishers[stream_index].getNumSubscribers())
                {
                    ...
                    RCLCPP_DEBUG(logger_, "Publish %s stream", rs2_stream_to_string(frame.get_profile().stream_type()));
                }
            });
...
void RealSenseCameraComponent::updateStreamCalibData(const rs2::video_stream_profile &video_profile)
{
...
Eigen::Quaternionf RealSenseCameraComponent::rotationMatrixToQuaternion(float rotation[9]) const
{
    Eigen::Matrix3f m;
...
void RealSenseCameraComponent::publishStaticTransforms()
{
...
void RealSenseCameraComponent::alignFrame(
        ...
        for (int from_x = 0; from_x < from_intrin.width; ++from_x, ++from_pixel_index)
        {
            ...
            float depth =
                (from_stream_type ==
                 RS2_STREAM_DEPTH)
                    ? (depth_units *
                       ((const uint16_t *)p_from_frame)[from_pixel_index])
                    : 1.f;
            if (depth)
            {
                ...
                from_pixel[0] = from_x + 0.5f;
                from_pixel[1] = from_y + 0.5f;
                rs2_deproject_pixel_to_point(from_point, &from_intrin, from_pixel, depth);
                rs2_transform_point_to_point(other_point, &from_to_other, from_point);
                rs2_project_point_to_pixel(other_pixel, &other_intrin, other_point);
...
rs2_extrinsics RealSenseCameraComponent::getRsExtrinsics(
...
void RealSenseCameraComponent::publishAlignedDepthImg(rs2::frame depth_frame, const rclcpp::Time &t)
{
    rs2::align align(RS2_STREAM_COLOR);
    _aligned_frameset = depth_frame.apply_filter(align);
    rs2::depth_frame aligned_depth = _aligned_frameset.get_depth_frame();
    rs2::video_frame aligned_color = _aligned_frameset.get_color_frame(); // add: 2020.3.16
    ...
    auto color_image = cv::Mat(cv::Size(aligned_color.get_width(), aligned_color.get_height()), _image_format[COLOR],
                               const_cast<void *>(aligned_color.get_data()), cv::Mat::AUTO_STEP);//comment 2020.3.16

    // add 2020.3.17 : Convert depth data to 8UC1 from 16SC1.
    // Depth accuracy (resolution) drops to about 2 cm to convert from short to uchar.
    cv::Mat depth_image8;
    depth_image.convertTo(depth_image8, CV_8UC1, 255.0/5000);

    // Create RGB-D 4ch image.
    cv::Mat aligned_rgbd;
    std::vector<cv::Mat> aligned_rgbd_element;
    cv::split(color_image, aligned_rgbd_element);
    aligned_rgbd_element.push_back(depth_image8);
    cv::merge(aligned_rgbd_element, aligned_rgbd);

    cv::cvtColor(aligned_rgbd, aligned_rgbd, cv::COLOR_BGR2RGB);//comment 2020.3.16
    //cv::imshow("aligned_depth", depth_image*8.0); //comment 2020.3.16
    cv::imshow("aligned_color", aligned_rgbd); //comment 2020.3.16
    cv::waitKey(1); //comment 2020.3.16

    // comment 2020.3.17 : 画像中央の距離を出力 [m]
    std::cout << aligned_depth.get_distance(vf.get_width()/2, vf.get_height()/2) <<
     ": " << depth_image.at<short>(vf.get_height()/2, vf.get_width()/2)/16 <<
     ": " << aligned_rgbd.at<cv::Vec4b>(vf.get_height()/2, vf.get_width()/2)[3]*5.0/255 <<
     ": " << aligned_rgbd.at<cv::Vec4b>(vf.get_height()/2, vf.get_width()/2) << std::endl;

    //auto vf_color = aligned_color.as<rs2::video_frame>();
    //std::cout << "vf_color.get_bytes_per_pixel() :" << vf_color.get_bytes_per_pixel() << std::endl;
    ...
    //           .toImageMsg();
    img = cv_bridge::CvImage(
              std_msgs::msg::Header(), sensor_msgs::image_encodings::TYPE_8UC4, aligned_rgbd)
              .toImageMsg();
...
void RealSenseCameraComponent::publishPCTopic(const rclcpp::Time &t)
{
    ...
    for (int y = 0; y < depth_intrinsics.height; ++y)
    {
        for (int x = 0; x < depth_intrinsics.width; ++x)
        {
            ...
            //std::cout << scaled_depth << std::endl; //comment 2020.3.16
...
void RealSenseCameraComponent::publishAlignedPCTopic(const rclcpp::Time &t)
{
    rs2::depth_frame aligned_depth = _aligned_frameset.get_depth_frame();
...
Extrinsics RealSenseCameraComponent::rsExtrinsicsToMsg(const rs2_extrinsics &extrinsics) const
{
...
Extrinsics RealSenseCameraComponent::getFisheye2ImuExtrinsicsMsg()
{
...
Extrinsics RealSenseCameraComponent::getFisheye2DepthExtrinsicsMsg()
{
    auto &fisheye = _enabled_profiles[FISHEYE].front();
...
IMUInfo RealSenseCameraComponent::getImuInfo(const stream_index_pair &stream_index)
{
    IMUInfo info;
...
void RealSenseCameraComponent::tryGetLogSeverity(rs2_log_severity &severity) const
{
...
void RealSenseCameraComponent::publishFrame(rs2::frame f, const rclcpp::Time &t)
{
    //std::cout << f.get_profile().stream_type() << " "; //comment 2020.3.16
    ...
    //     0 != image_publisher.getNumSubscribers())
    {
        ...
        if (f.is<rs2::video_frame>())
        {
            ...
            //std::cout << bpp << std::endl;//comment 2020.3.16
        }
...
bool RealSenseCameraComponent::getEnabledProfile(
